...
int main() {
	...
	while (true) {
		...
		for (int x = 0; x < X; x++) {
			...
			if (DistW <= DEPTH / 6.0f)			ShadeFringle = '!';			// 2591
			...
			if (DistW <= DEPTH / 6.0f)			ShadeWall = 'O';			// 88
			else if (DistW < DEPTH / 4.0f)		ShadeWall = 'Z';			// 93
			else if (DistW < DEPTH / 3.0f)		ShadeWall = 'V';			// 92
			else if (DistW < DEPTH)				ShadeWall = 'Y';			// 91
